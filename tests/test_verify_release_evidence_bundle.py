...
def _hermes_bundle_json() -> dict[str, object]:
    return {
        "quickstart": {
            ...
            "validate_runtime_transport": {"command": "python scripts/run_mcp_transport_smoke.py"},
            "audit_index_visibility": {"command": "python scripts/audit_mcp_index_visibility.py"},
            "run_local_stdio_server": {"command": "reg-rag-mcp-server"},
            "chatgpt_desktop_local": {"command": "powershell -File connect_mcp_client.ps1"},
            "claude_desktop": {"command": "powershell -File connect_mcp_client.ps1"},
            "codex": {"command": "codex mcp get policy-mcp"},
            "claude_code": {"command": "powershell -File claude_code_add_stdio.ps1"},
            "chatgpt_remote": {"command": "powershell -File validate_chatgpt_remote_mcp.ps1"},
            "vercel_https": {"command": "powershell -File validate_chatgpt_remote_mcp.ps1"},
            "claude_remote": {"command": "powershell -File claude_code_add_http.ps1"},
            "codex_claude_team": {"command": "powershell -File validate_client_config_smoke.ps1"},
        },
        "chatgpt_desktop_local": {"ready": True},
        "claude_desktop": {"ready": True},
        "claude_code": {"ready": True},
        "chatgpt_remote": {"ready": True},
        "claude_remote": {"ready": True},
    }
