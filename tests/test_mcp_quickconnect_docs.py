...
class McpQuickConnectDocsTests(unittest.TestCase):
    def test_quickconnect_doc_lists_common_client_paths(self) -> None:
        ...
        self.assertIn("공식 `user` scope 서버는 `~/.claude.json`에 저장", text)
        ...
        self.assertIn("Settings > Plugins", text)
        self.assertIn("search(query)", text)
        self.assertIn("fetch(id)", text)
        self.assertIn("MCP OAuth 2.1", text)
        self.assertIn("--chatgpt-oauth-ready", text)
        self.assertNotIn("Settings > Apps > Advanced Settings", text)
        self.assertNotIn("Settings > Apps > Create", text)
    ...
    def test_client_config_doc_explains_warm_cache_and_latency_fields(self) -> None:
        ...
        self.assertIn("CODEX_AGENT_CONNECT_PROMPT.md`는 로컬 파일·터미널 권한", text)
        self.assertIn("직접 설정이 기본", text)
        self.assertIn("일반 대화 프롬프트에 붙여넣지 않습니다", text)
        self.assertIn("Settings > MCP servers > Add server", text)
        self.assertIn("CODEX_AGENT_CONNECT_PROMPT.md", text)
        self.assertIn("CHATGPT_DESKTOP_CONNECT_GUIDE.md", text)
        self.assertIn("CLAUDE_CODE_AGENT_CONNECT_PROMPT.md", text)
        self.assertIn("Claude Desktop은 전용 BAT가 기본", text)
        self.assertIn("Codex CLI는 `Codex에 연결하기.bat`", text)
        self.assertIn("Desktop에 없는 MCP 기능을 활성화하지는 않으므로", text)
        self.assertIn("Settings > Security and login", text)
        self.assertIn("Settings > Plugins", text)
        self.assertIn("search(query)", text)
        self.assertIn("fetch(id)", text)
        ...
        self.assertNotIn("Settings > Apps > Advanced Settings", text)
        self.assertNotIn("Settings > Apps > Create", text)
        self.assertIn("검토·배포된 marketplace 플러그인 설치는 개발자 모드 앱 생성과 별도 단계", text)
        self.assertIn("Customize > Connectors", text)
