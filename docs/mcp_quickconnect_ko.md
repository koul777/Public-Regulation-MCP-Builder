...
PR MCP Builder에서 전처리와 승인까지 끝낸 MCP를 ChatGPT Desktop 로컬 direct MCP, ChatGPT 원격 MCP, Codex CLI, Claude Desktop, Claude Code, Claude API에 빠르게 연결하기 위한 최소 절차입니다.
...
- `direct_config_registered`: ChatGPT Desktop용 보조 BAT 또는 Codex CLI가 읽는 로컬 MCP 설정에 direct 항목 기록 성공
...
`--public-url`은 ChatGPT 원격 MCP나 Claude API처럼 원격 HTTPS MCP가 필요한 클라이언트에만 필요합니다. ChatGPT Desktop 로컬 direct MCP, Codex CLI, 로컬 Claude Desktop/Claude Code만 쓸 때는 생략합니다. Claude Code의 stdio 서버는 `--scope user`로 등록되므로 같은 사용자의 프로젝트 전체에서 보입니다. Streamlit 운영 화면에서는 MCP 영역의 `MCP로 쓸 파일 묶음 만들기` 버튼으로 같은 번들을 만들 수 있습니다.
...
ChatGPT Desktop 사용자는 프로그램 생성 결과 화면의 `CHATGPT_DESKTOP_CONNECT_GUIDE.md` 코드 상자에 표시된 Name·STDIO·Command·Working directory·Arguments를 `Settings > MCP servers > Add server`에 입력합니다. ZIP 안의 원본 GUIDE에 `<PROGRAM_BUNDLE_DIR>`이 보이면 이식용 자리표시자이므로 그대로 복사하지 말고, 추출한 폴더를 프로그램에서 다시 선택해 안내를 표시하거나 Desktop 전용 BAT를 사용합니다. Codex CLI와 Claude Code 사용자는 대상별 에이전트 연결 요청문을 복사해 실행합니다. ChatGPT Desktop 내장 등록이 어렵거나 에이전트에 로컬 실행 권한이 없을 때만 대상별 BAT를 사용합니다.
...
- Claude (HTTPS MCP): `Claude HTTPS에 연결하기.bat`와 `claude_api_fragment.json` 사용

ChatGPT Desktop은 내장 설정 등록 후, Codex CLI와 Claude Code는 설치·로더 검증 후 해당 앱을 완전히 종료·재실행하고 새 대화 또는 task에서 다음처럼 호출합니다.

```text
먼저 /mcp로 aks_mcp가 연결됨으로 보이는지 확인한 뒤, aks_mcp MCP의 연결 상태와 사용 가능한 규정 도구를 보여줘.
```

Claude Desktop BAT는 설치된 사용자 설정으로 `initialize`·`tools/list`·`get_index_status`까지 직접 검증합니다. 이 성공은 Claude Desktop 자체 로더나 현재 대화의 도구 노출 성공을 뜻하지 않습니다. 앱을 완전히 종료·재실행한 뒤 새 대화에서 입력창의 `+` → Connectors에서 서버를 확인하고 MCP 이름을 포함해 실제 `get_index_status` 호출을 요청합니다. Claude Desktop에는 위 `/mcp` 지시를 적용하지 않습니다.

같은 이름으로 번들을 다시 생성하면 ChatGPT Desktop은 새 안내 값을 기존 `Settings > MCP servers` 항목에 반영하고, Codex CLI·Claude Code·Claude Desktop은 대상별 연결 절차를 다시 실행해 기존 항목을 교체합니다. 새 번들은 현재 승인된 전체 청크를 다시 내보내므로 추가 규정과 개정판도 같은 `aks_mcp`에서 조회됩니다. 저장 폴더가 바뀌었다면 반드시 새 폴더 기준 경로로 갱신합니다.
...
Work/Codex 플러그인 배포를 명시적으로 선택한 경우에만 direct 설정과 겹치지 않는 격리 환경에서 `-InstallPackage -Target chatgpt-desktop-local -InstallChatGptDesktopPlugin`을 사용합니다. 일반 ChatGPT Desktop 연결에는 위의 direct 등록을 사용합니다.
...
## 3. ChatGPT Desktop 로컬 MCP와 Codex CLI

프로그램 생성 결과 화면의 `CHATGPT_DESKTOP_CONNECT_GUIDE.md` 코드 상자에 표시된 Name·STDIO·Command·Working directory·Arguments를 ChatGPT Desktop의 `Settings > MCP servers > Add server`에 입력하고 Save한 뒤 Restart합니다. ZIP 원본의 `<PROGRAM_BUNDLE_DIR>` 자리표시자는 입력값이 아닙니다. 새 대화에서 `/mcp`로 서버 이름을 확인하고 실제 `get_index_status`를 호출합니다. 내장 메뉴가 없거나 수동 입력이 어려울 때만 보조 수단인 `ChatGPT Desktop에 연결하기.bat`를 Windows 탐색기에서 실행합니다. `@MCP이름` 반복 입력은 설치나 연결 확인을 대신하지 않습니다. 생성된 plugin package는 별도 플러그인 배포가 명시적으로 필요할 때만 사용합니다.

ChatGPT 대화 화면이 로컬 direct MCP를 노출하지 않는 제품 구성에서는 아래의 `chatgpt-remote` HTTPS 또는 Secure MCP Tunnel 방식을 사용합니다. 생성 플러그인은 Work/Codex 배포가 명시적으로 필요할 때만 쓰는 선택 산출물입니다. `Codex 플러그인 MCP 입력값.txt`는 레거시 파일명을 유지한 Codex CLI 수동 호환 정보이며, 기본 연결은 `CODEX_AGENT_CONNECT_PROMPT.md`, 실행 권한이 없을 때의 보조 연결은 `Codex에 연결하기.bat`입니다.
...
이 절차는 위의 ChatGPT Desktop 로컬 direct MCP와 별개입니다. ChatGPT 개발자 모드 앱/플러그인은 ChatGPT에서 접근 가능한 인증된 HTTPS `/mcp` endpoint가 필요하며 localhost에 직접 연결하지 않습니다. 생성된 `chatgpt-remote` 프로필은 외부 응답 경계를 위해 `chatgpt-data` 프로필(search/fetch)만 명시적으로 노출합니다. 외부 connector 응답에는 `source_record_id`, `source_file_id`, `approval_review_batch_manifest_path` 같은 내부 운영 metadata가 포함되면 안 됩니다.

```powershell
powershell -ExecutionPolicy Bypass -File reports/mcp_connection_bundle/run_chatgpt_data_server.ps1
```

ChatGPT Settings > Security and login에서 Developer mode를 켠 뒤 Settings > Plugins 또는 `https://chatgpt.com/plugins`에서 개발자 모드 앱을 만듭니다. 앱 이름은 프로그램에서 입력한 MCP 이름으로 지정하고 `chatgpt_connector.json`의 `connector_url`을 등록합니다. 생성 시 표시되는 도구 목록에서 `search`와 `fetch`를 확인합니다. `get_index_status` 같은 내부 진단은 로컬/full 프로필에서만 실행합니다. 새 ChatGPT 대화에서 `+` > `More`를 열어 만든 앱을 선택한 뒤 실제 `search`와 `fetch` 호출로 연결을 확인합니다.

배포된 HTTPS endpoint 자체는 다음 스크립트로 검증합니다. 이 스크립트가 성공해도 Plugins의 도구 목록 발견 및 ChatGPT 대화 첨부 상태는 별도입니다.
...
- ChatGPT 개발자 모드 Plugins: https://developers.openai.com/apps-sdk/deploy/connect-chatgpt
- Claude API MCP connector: https://docs.anthropic.com/en/docs/agents-and-tools/mcp-connector
- Claude Code MCP: https://docs.anthropic.com/en/docs/claude-code/mcp
- 대상별 연결: ChatGPT Desktop은 `CHATGPT_DESKTOP_CONNECT_GUIDE.md`의 실제 값을 `Settings > MCP servers > Add server`에 등록하고, Codex CLI와 Claude Code는 압축 해제한 번들을 로컬 작업공간으로 열어 대상별 `*_AGENT_CONNECT_PROMPT.md`를 실행합니다. 등록 또는 설치 후 앱을 완전히 종료·재실행하고 새 대화 또는 task에서 `/mcp`로 `aksmcp`와 실제 도구 호출을 확인합니다. ChatGPT Desktop용 BAT는 내장 메뉴가 없거나 수동 등록이 어려울 때만 쓰는 보조 수단입니다. Claude Desktop은 전용 BAT가 기본이고 `/mcp` 공통 지시에서 제외합니다. 선택형 플러그인은 별도 플러그인 배포가 필요할 때만 사용합니다. HTTPS는 공개 `/mcp` URL을 사용하고, Tunnel은 OpenAI Tunnel ID와 API 키를 실행 환경에 설정합니다. 사용자는 `.ps1` 명령을 직접 조합하지 않습니다.
