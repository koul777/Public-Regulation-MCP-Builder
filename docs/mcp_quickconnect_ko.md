...
- `end_to_end_verified`: 해당 smoke 보고서에서 Claude 로컬 `full`은 `initialize`, `tools/list`, `get_index_status`; ChatGPT Desktop·Codex·외부 `chatgpt-data`는 `initialize`, `tools/list`, `search`, `fetch` 전송 계약이 모두 성공함. Desktop 도구 scan이나 현재 대화 첨부 완료를 뜻하지 않음
...
생성된 번들이 각 로컬 설정에서 바로 실행되는지 보려면 클라이언트 설정 파일 자체를 실행하는 smoke를 추가로 돌립니다. 이 검사는 Codex, Claude Desktop, ChatGPT Desktop 플러그인 `.mcp.json`을 읽어 실제 stdio MCP 프로세스를 띄웁니다. Claude 로컬 `full`은 `get_index_status`, ChatGPT Desktop·Codex의 `chatgpt-data`는 `search`·`fetch`까지 호출합니다.
...
아래 순서는 프로그램의 연결 대상 표시 순서와 같습니다. Codex CLI는 직접 설정/BAT, Claude Code는 선택적 에이전트 연결 요청문 또는 BAT, Claude Desktop은 전용 BAT를 사용합니다. ChatGPT Desktop은 프로그램 생성 결과의 `CHATGPT_DESKTOP_CONNECT_GUIDE.md`에 표시된 값을 `Settings > MCP servers > Add server`에 직접 입력합니다. ZIP 원본의 `<PROGRAM_BUNDLE_DIR>`은 그대로 복사하지 않습니다.

- Claude Code: `CLAUDE_CODE_AGENT_CONNECT_PROMPT.md` 우선, `Claude Code에 연결하기.bat` 보조
- Codex CLI: `Codex에 연결하기.bat` 우선, `codex_config_snippet.toml` 직접 설정 가능; `CODEX_AGENT_CONNECT_PROMPT.md`는 선택적 로컬 자동화 자료
...
ChatGPT Desktop과 Codex CLI에는 개인정보 노출을 줄인 `chatgpt-data` 프로필만 생성합니다. 설치·등록 후 앱을 완전히 종료·재실행하고 새 대화 또는 task에서 다음처럼 확인합니다.

```text
먼저 /mcp로 aks_mcp가 연결됨으로 보이는지 확인한 뒤, aks_mcp MCP의 search 도구로 인사규정을 찾고 반환된 첫 번째 id를 fetch 도구로 조회해 원문과 출처를 보여줘.
```

Claude Code·Claude Desktop의 로컬 운영자용 `full` 프로필은 별도로 `get_index_status`를 확인할 수 있습니다. 연결 설정, 로컬 경로, 토큰, API 키, tunnel ID는 일반 대화 프롬프트에 붙여넣지 않습니다. 비밀값은 승인된 환경변수·secret manager 또는 OAuth에만 둡니다. 위의 `search`·`fetch` 문장은 설정값이나 비밀값이 없는 일반 사용 질의이므로 연결 후 대화에서 사용해도 됩니다.
...
같은 이름으로 번들을 다시 생성하면 Codex CLI는 전용 BAT를 다시 실행하거나 TOML 설정을 직접 교체하고, Claude Code는 요청문 또는 BAT, Claude Desktop은 전용 BAT를 다시 실행합니다. ChatGPT Desktop은 새 안내 값을 기존 `Settings > MCP servers` 항목에 반영합니다. 새 번들은 현재 승인된 전체 청크를 다시 내보내므로 추가 규정과 개정판도 같은 `aks_mcp`에서 조회됩니다. 저장 폴더가 바뀌었다면 모든 로컬 대상에서 새 폴더 기준 경로로 갱신합니다.
...
이 절차는 위의 ChatGPT Desktop 로컬 direct MCP와 별개입니다. ChatGPT 개발자 모드 앱은 ChatGPT에서 접근 가능한 인증된 HTTPS `/mcp` endpoint가 필요하며 localhost에 직접 연결하지 않습니다. 생성된 `chatgpt-remote` 프로필은 외부 응답 경계를 위해 `chatgpt-data` 프로필만 사용하며, 입력 스키마는 정확히 `search(query)`와 `fetch(id)`입니다. 검색 결과의 인용 주소는 사용자가 열 수 있는 절대 HTTP(S) 원문 URL 또는 빈 문자열이어야 합니다. 외부 응답에는 `source_record_id`, `source_file_id`, `approval_review_batch_manifest_path` 같은 내부 운영 metadata와 `govreg://` 로컬 URI가 포함되면 안 됩니다.

직접 HTTPS 앱의 인증은 MCP OAuth 2.1이어야 합니다. ChatGPT는 정적 API 키나 운영자가 만든 `MCP_AUTH_TOKEN`을 입력받지 않으므로, 이 토큰만 설정한 endpoint는 도구 scan에서 인증되지 않습니다. OAuth discovery·PKCE·audience·scope·callback 검증을 마친 경우에만 `--chatgpt-oauth-ready`로 번들을 다시 생성합니다. OAuth gateway가 없으면 아래 Secure MCP Tunnel 경로를 사용합니다.

```powershell
powershell -ExecutionPolicy Bypass -File reports/mcp_connection_bundle/run_chatgpt_data_server.ps1
```

ChatGPT `Settings > Security and login`에서 Developer mode를 켠 뒤 `Settings > Plugins` 또는 `https://chatgpt.com/plugins`의 `+`에서 개발자 앱을 만듭니다. 앱 이름은 프로그램에서 입력한 MCP 이름으로 지정하고 MCP server URL에 `chatgpt_connector.json`의 HTTPS `connector_url`을 등록합니다. 생성 시 표시되는 도구 목록에서 `search`와 `fetch`를 확인합니다. `get_index_status` 같은 내부 진단은 로컬/full 프로필에서만 실행합니다. 새 ChatGPT 대화의 `+ > More`에서 만든 앱을 선택한 뒤 실제 `search(query)`와 `fetch(id)` 호출로 연결을 확인합니다.
...
실행 전에 `CONTROL_PLANE_API_KEY`와 `OPENAI_TUNNEL_ID`를 승인된 환경변수로 설정합니다. 값을 스크립트 파일에 저장하지 않습니다. ChatGPT `Settings > Security and login`에서 Developer mode를 켠 뒤 `Settings > Plugins` 또는 `https://chatgpt.com/plugins`의 `+`에서 앱을 만들고 Connection을 Tunnel로 선택합니다. 새 대화에서는 `+ > More`에서 앱을 첨부합니다. 공개 HTTPS 방식은 같은 개발자 앱 화면에서 MCP server URL에 `/mcp` 주소를 넣으며, 검토·배포된 marketplace 플러그인 설치는 별도 단계입니다.
