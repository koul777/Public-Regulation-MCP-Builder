...
- `direct_config_registered`: ChatGPT Desktop·Codex 공유 `config.toml`에 direct MCP 항목 기록 성공
...
ChatGPT Desktop Work/Codex, Codex, Claude Code 사용자는 생성 결과 아래의 대상별 에이전트 연결 요청문을 먼저 복사해 붙여넣습니다. 에이전트가 doctor, 설치, 각 클라이언트의 `mcp get` 검증을 수행합니다. 에이전트에 로컬 파일·터미널 권한이 없을 때만 해당 BAT를 실행하며, 사용자가 `.ps1`을 직접 조합할 필요는 없습니다.

- ChatGPT Desktop 로컬 direct MCP: `CHATGPT_DESKTOP_AGENT_CONNECT_PROMPT.md` 우선, `ChatGPT Desktop에 연결하기.bat` 보조
- Codex CLI 호환: `CODEX_AGENT_CONNECT_PROMPT.md` 우선, `Codex에 연결하기.bat` 보조
- Claude Code: `CLAUDE_CODE_AGENT_CONNECT_PROMPT.md` 우선, `Claude Code에 연결하기.bat` 보조
- Claude Desktop: `Claude Desktop에 연결하기.bat` 기본

ChatGPT Desktop, Codex, Claude Code는 설치·로더 검증이 끝난 뒤 해당 앱을 완전히 종료·재실행하고 새 대화 또는 task에서 다음처럼 호출합니다.

```text
먼저 /mcp로 aks_mcp가 연결됨으로 보이는지 확인한 뒤, aks_mcp MCP의 연결 상태와 사용 가능한 규정 도구를 보여줘.
```

Claude Desktop은 BAT 설치 검증 후 앱을 완전히 종료·재실행하고 MCP 이름을 포함해 호출합니다. Claude Desktop에는 위 `/mcp` 지시를 적용하지 않습니다.
...
비대화형 보조 경로로 ChatGPT Desktop/Codex 공유 direct 설정과 Claude Code를 바로 등록할 수도 있습니다.
...
## 3. ChatGPT Desktop 로컬 direct MCP와 Codex CLI

압축을 푼 번들 폴더를 ChatGPT Desktop의 Work/Codex 로컬 작업공간으로 연 뒤 `CHATGPT_DESKTOP_AGENT_CONNECT_PROMPT.md`를 붙여넣습니다. 에이전트는 doctor를 실행하고 ChatGPT Desktop·Codex가 공유하는 `~/.codex/config.toml`의 `[mcp_servers.*]` direct stdio 항목을 백업·갱신한 뒤 `codex mcp get --json`으로 현재 번들의 launcher와 data 경로를 검증합니다. 일반 채팅처럼 로컬 파일·터미널 권한이 없는 화면에서는 보조 수단인 `ChatGPT Desktop에 연결하기.bat`가 같은 direct 등록을 실행합니다. 앱을 완전히 종료하고 다시 실행한 뒤 새 대화에서 `/mcp`로 서버를 먼저 확인합니다. `@MCP이름` 반복 입력은 설치나 연결 확인을 대신하지 않습니다. 생성된 plugin package는 Work/Codex 플러그인 배포가 명시적으로 필요할 때만 사용합니다. 플러그인 manifest의 `mcpServers`는 `./.mcp.json`을 가리키며, companion은 현재 Codex 플러그인 검증기와 app-server가 함께 승인하는 `mcpServers` 컨테이너를 사용합니다.
...
- 에이전트 우선 연결: ChatGPT Desktop, Codex, Claude Code는 압축 해제한 번들을 로컬 작업공간으로 열고 대상별 `*_AGENT_CONNECT_PROMPT.md`를 실행합니다. doctor·설치·로더 검증이 끝난 뒤 해당 앱을 완전히 종료·재실행하고 새 대화 또는 task에서 `/mcp`로 `aksmcp`를 확인합니다. Windows 번들의 클라이언트별 BAT는 로컬 실행 권한이 없을 때만 쓰는 보조 수단입니다. Claude Desktop은 전용 BAT가 기본이고 `/mcp` 공통 지시에서 제외합니다. ChatGPT Desktop은 direct config가 기본이며 플러그인은 Work/Codex 배포용 선택 산출물입니다. HTTPS는 공개 `/mcp` URL을 사용하고, Tunnel은 OpenAI Tunnel ID와 API 키를 실행 환경에 설정합니다. 사용자는 `.ps1` 명령을 직접 조합하지 않습니다.
