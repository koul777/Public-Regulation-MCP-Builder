...
- `codex_config_snippet.toml`: Codex CLI·Codex IDE 설정
...
- `chatgpt_desktop_local_mcp.json`: 이전 버전 호환용 **지원 종료 경고 파일**. 직접 연결에 사용하지 않음
...
`codex_config_snippet.toml`의 `[mcp_servers.<이름>]` 블록을 `~/.codex/config.toml`에 한 번만 반영하고 사용 중인 CLI 또는 IDE 확장을 완전히 재시작한 뒤 `/mcp`와 실제 `search`·`fetch` 호출을 확인합니다. 직접 등록에는 `codex plugin list`가 필요하지 않습니다.
...
독립 전달 ZIP을 다른 Windows PC에서 처음 쓸 때는 Python 3.11 이상을 설치하고 ZIP을
완전히 푼 뒤, 그 폴더의 `install_local_package.ps1`을 먼저 실행해야 합니다. 이 스크립트가
ZIP에 포함된 정확한 wheel을 설치한 다음에만 `run_mcp_stdio_server.ps1`과 앱 등록·진단을
실행합니다. wheel 파일만으로는 Python 실행 환경이 생기지 않습니다. Python이 없는 PC라면
portable 실행판 전체를 옮겨 그 PC에서 MCP 묶음을 다시 생성하는 방법을 사용하세요.
...
ChatGPT는 로컬 MCP 서버에 직접 연결하지 않습니다. `chatgpt_desktop_local_mcp.json`이
남아 있어도 이전 번들 호환용 지원 종료 경고 파일이므로 등록하지 마세요. ChatGPT 웹의
Developer mode에서 원격 HTTPS MCP 앱을 만들거나, 사설망·개발 PC 서버에는 OpenAI
Secure MCP Tunnel을 별도로 구성합니다.
...
하나의 Vercel 배포와 하나의 `/mcp` endpoint를 ChatGPT 웹·Codex와 Claude가 함께
사용할 수 있습니다. 클라이언트별 서버를 따로 만들지 않습니다. ChatGPT 웹은 Developer
mode의 Apps 설정에서 원격 MCP 앱을 만들고, Codex는 `~/.codex/config.toml`, Claude는
해당 Connector 등록 화면에 같은 URL을 입력합니다.

Codex 원격 설정의 핵심은 다음 두 값입니다.
...
- Codex 등 bearer 환경변수를 지원하는 MCP 클라이언트: Vercel Secret
...
Codex 연결은 토큰 값을 설정 파일에 직접 쓰지 않고 환경변수 이름만
`bearer_token_env_var`로 등록합니다. ChatGPT 웹은 워크스페이스가 승인한 원격 MCP 앱과
OAuth 정책을 따르며, 이 번들이 ChatGPT 계정에 앱을 자동 등록하지는 않습니다.
...
- 거부·상위대체 청크의 본문과 원본 검토 저널은 제외합니다. `data/repository/omission_disposition_snapshot.json`에는
  결정 ID·시각·내용 해시·포함/누락 분류만 최소 감사 증거로 봉인하며, 검토 사유·담당자·원문 경로는
  넣지 않습니다. 이 감사 파일은 MCP 검색·조회 응답으로 제공하지 않습니다.
