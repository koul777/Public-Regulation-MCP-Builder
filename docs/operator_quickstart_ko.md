...
고른다. **이 PC의 로컬 Qwen 챗봇으로 바로 질문**은 이 프로그램에서 규정에 바로 묻는
경로이고, **ChatGPT·Claude·Codex에 MCP로 연결**은 외부 AI 앱에 같은 규정 도구를
...
3. `③ 검수하고 승인`은 세 단계뿐이다. **1단계** 규정 디렉터리에서 **규정 열기**를 눌러
   ...
   **승인·색인 완료**가 함께 표시되거나 명시 반려로 처리 방향이 결정돼야 다음 단계로
   이동한다.
4. ④는 첫 선택에 따라 나뉜다. **로컬 Qwen**을 골랐다면 `④ Qwen 규정 챗봇·AI 연결`의
   Qwen 탭에서 아래 다섯 단계를 따른다: (1) 승인·색인 준비 상태 확인, (2) **Qwen3 8B
   챗봇 켜기**, (3) **Qwen 연결 확인**, (4) 예시 또는 직접 질문 입력, (5) 답변과 근거
   조문·인용 함께 확인. Qwen3 8B 답변은 Qwen3 4B 주장 감사와 결정적 인용 검증을 거친다.
   **MCP**를 골랐다면 메뉴가 `④ MCP 생성·외부 AI 연결`로 보인다. 사용할 외부 AI 앱과
   ...
   반환하는지 확인한다. MCP를 골라도 로컬 Qwen 탭은 그대로 사용할 수 있다.
