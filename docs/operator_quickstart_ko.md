...
4. ④는 첫 선택에 따라 나뉜다. **로컬 Qwen**을 골랐다면 `④ Qwen 규정 챗봇·AI 연결`에서
   **독립 Qwen 챗봇 실행**을 누른다. 새 브라우저 창에서 아래 다섯 단계를 따른다:
   (1) 기관 선택, (2) 규정별 승인·색인 준비 상태 확인, (3) **질문 가능** 규정 하나 선택,
   (4) **Ollama · qwen3:8b 연결 확인** 후 질문 입력, (5) 답변과 근거 조문·인용 함께 확인.
   목록에는 전처리가 끝난 규정이 보이지만 승인·색인이 완전히 끝난 항목만 선택할 수 있다.
   질문 중에는 검색·Qwen 답변·인용 검증 단계와 경과 시간이 실시간으로 표시된다. Qwen3 8B
   답변은 Qwen3 4B 주장 감사와 결정적 인용 검증을 거친다. 빌더를 닫고 챗봇만 다시 열려면
   저장소 루트에서 `RUN_QWEN_CHAT.bat`를 실행한다.
