...
조문은 의미가 비슷한 다른 조문으로 대체하지 않고 근거 없음으로 처리한다. 복수 조문 비교,
항·호만 지정한 질문, 기준일이 있는 질문과 일반 자연어 질문은 기존 하이브리드 검색을 사용한다.
일반 검색의 CPU 재랭커는 한 번 적재한 모델을 재사용하고, 기본 `top_k=5`에서는 상위 후보
10개까지만 재정렬한다.

진행 화면에서 `2/5 승인된 규정 조항을 찾고 있습니다`가 수 초 이상 계속되면 구버전 챗봇
프로세스인지 먼저 확인하고 독립 Qwen 앱만 다시 실행한다. 빌더와 MCP 서버는 재시작할 필요가
없다. 정확 조문 검색이 끝난 뒤의 `4/5 Qwen3 8B` 답변 생성과 `4/5` 근거 감사 시간은 로컬
CPU·GPU와 모델 적재 상태에 따라 달라질 수 있다.
