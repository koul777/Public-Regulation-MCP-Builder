...
- 운영자가 명시적으로 승인한 데이터만 MCP 조회 대상으로 공개한다. 사람 검토는 권고하되 완료 여부로 최종 승인 버튼을 막지 않고, 미검수 승인은 감사 이벤트로 구분한다.
...
3. 시스템이 전처리 결과와 사람 검토 권고를 제시하고, 운영자는 필요한 부분을 수정할 수 있다.
4. 운영자가 MCP 공개 가능 청크를 명시적으로 승인한다. 검토를 생략한 경우 사유와 감사 이벤트를 기록한다.
...
외부 ChatGPT 개발자 모드 앱/플러그인 연결은 MVP 기본 범위가 아니다. 외부 연결이 승인된 경우 공개 가능 데이터 또는 별도 승인된 망연계 환경의 runtime bundle을 Vercel에 배포하고, 최종 HTTPS `/mcp` 주소와 승인된 인증을 Connector에 직접 등록한다. 내부망 MCP 서버나 로컬 폴더를 외부 클라이언트에 직접 노출하지 않는다.
