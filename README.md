...
- `4-4-3. 여비규정.hwp`처럼 **한 규정 안의 별표 제목**이 있는 문서를 합본 규정집으로
  ...
  의심합니다.
- 초보자 안내를 4개의 큰 설명에서 **27개의 세부 확인 절차**로 나눴습니다. 왼쪽에는
...
- 검수 화면에서는 AI 제안별 판단 → AI 검증 결과 확인 → **왼쪽 원본 규정과 오른쪽
  ...
  안내합니다. 승인만 끝나고 색인이 남은 경우에는 `이미 승인된 내용 AI에 등록만 실행`
  버튼을 별도로 가리킵니다.
- MCP 화면에서는 규정 범위 → 사용할 AI 앱 → 저장 위치·방식·MCP 이름 → 파일 묶음
  생성을 각각 확인합니다. 이후 선택한 방식에 맞춰 Claude Code, Codex CLI·IDE,
  Claude Desktop 로컬 연결 또는 ChatGPT·Claude Vercel HTTPS 연결 절차만 보여 줍니다.
...
- 500페이지 합성 텍스트 PDF를 같은 바이트로 다시 측정한 결과 16.981초, 초당 29.445페이지,
  ...
  보장하는 값은 아닙니다.
- 저장소 전체 3,011개 테스트, 핵심 업로드·계층 MCP·성능 회귀 315개 테스트, 격리된 소스
...
| --- | --- | --- |
| ① | `① 문서 올려서 전처리`에서 **문서 업로드** → 자동 인식 정보 확인 → **AI로 의심 구간 추가 검수 (선택)** 사용 여부 결정 → **전처리 시작** 순서로 진행 | **전처리 완료** |
| ② | `② 결과 확인`에서 원문·문서 구조·**정리된 내용(청크)** 확인과 품질 경고·이슈 확인을 각각 완료 | 두 확인란이 모두 완료됨 |
| ③ | AI 제안별 판단과 AI 결과 확인 → **왼쪽 원본 규정 / 오른쪽 전처리·수정 결과** 비교 → 사람 결과 확인 → 모든 청크를 **승인하고 색인** 또는 **이 청크 반려**로 처리 | **승인·색인 완료** 또는 명시 반려로 처리 방향 결정 |
| ④ | 규정 범위 → AI 앱 → 저장 설정·**생성할 MCP 이름 (필수 입력)** → **MCP로 쓸 파일 묶음 만들기** → **앱별 등록·연결 진단** → `list_regulations` → `search` → `fetch` 순서로 진행 | 여섯 개의 실제 연결 확인이 모두 완료됨 |
