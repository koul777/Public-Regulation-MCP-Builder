...
- 저장소 전체 회귀 테스트 **3,339개 통과**(14개 건너뜀)
...
4. `선택한 규정에 대해 질문하세요` 입력칸에 자연스럽게 질문합니다. 예:
   ...
   `그 절차에서 담당자는 누구야?`처럼 이어서 물을 수 있습니다. 기본 빠른 모드에서는 승인
   근거 검색, Qwen3 8B 짧은 답변, 결정론적 인용 검증과 경과 시간이 게이지에 표시됩니다.
   의미 단위 재검토가 필요하면 **Qwen3 4B 정밀 근거 감사**를 켭니다.
5. 답변만 읽고 끝내지 말고, 아래에 열린 **근거 조문**에서 규정명·조문 번호·인용문을
   원문과 함께 확인합니다. 기본 모드에서도 답변의 모든 `[E번호]`가 선언된 승인 검색 결과와
   정확히 일치해야 표시됩니다. 근거 없음 응답은 고정 문구로 제한됩니다. 정밀 감사 모드에서는
   이 검증에 Qwen3 4B의 의미 감사가 추가됩니다.
