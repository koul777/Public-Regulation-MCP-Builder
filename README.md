...
| ③ | 규정마다 결과 두 곳 확인 → AI 제안별 판단과 AI 결과 확인 → **왼쪽 원본 규정 / 오른쪽 전처리·수정 결과** 비교 → 사람 결과 확인 → 모든 청크를 **승인하고 색인** 또는 **이 청크 반려**로 처리 → 다음 미완료 규정에서 반복 | 선택한 모든 규정의 **승인·색인 완료** 또는 명시 반려로 처리 방향 결정 |
| ④ | MCP 원리·변환 과정 확인 → 규정 범위 → AI 앱 → 저장 설정·**생성할 MCP 이름 (필수 입력)** → **MCP로 쓸 파일 묶음 만들기** → **앱별 등록·연결 진단** → `list_regulations` → `search` → `fetch` 순서로 진행 | 여섯 개의 실제 연결 확인이 모두 완료됨 |
