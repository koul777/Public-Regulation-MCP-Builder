...
- 일반적인 자연어 질문은 기존 하이브리드 검색을 유지하되, 기본 설정의 재랭킹 후보를 최대
  50개에서 10개로 줄이고 CPU 재랭커를 재사용합니다.
- 검색 뒤 Qwen3 8B 답변 생성과 Qwen3 4B 근거 감사에는 PC 성능에 따른 시간이 추가로
  필요하지만, 화면의 진행 상태가 2/5에서 3/5·4/5로 이어져 검색과 답변 대기를 구분합니다.
...
- 질문을 보낸 뒤 화면이 멈춘 것처럼 보이지 않도록 **질문 분석 → 승인 근거 검색 → 문맥 구성 →
  Qwen3 8B 답변 → Qwen3 4B 감사 → 인용 검증**의 실제 단계, 진행 게이지와 경과 시간을 계속 표시합니다.
...
- 저장소 전체 회귀 테스트 **3,330개 통과**(14개 건너뜀)
...
- **이 PC의 로컬 Qwen 챗봇으로 바로 질문**(권장): 승인 뒤 **독립 Qwen 챗봇 실행**을
  눌러 별도 로컬 앱을 열고, 대화할 승인 규정을 선택합니다. 질문과 규정은 외부 API로 보내지 않습니다.
- **ChatGPT·Claude·Codex에 MCP로 연결**: 승인 뒤 ④가 **MCP 생성·외부 AI 연결**로
  표시됩니다. 사용할 앱을 골라 MCP 묶음을 만들고, 그 앱에서 `list_regulations` →
  `search` → `fetch`를 확인합니다. MCP를 선택해도 독립 로컬 Qwen 앱과 승인 RAG는 남아 있습니다.
...
빌더와 분리된 Qwen 앱에서 아래 다섯 단계만 차례대로 진행하세요. 이 과정은 MCP를 만들지 않으며, MCP 기능을
지우지도 않습니다. Qwen과 MCP는 같은 승인 로컬 RAG를 공유하므로 별도 RAG 구축은
필요 없습니다.

1. 빌더 왼쪽의 **독립 Qwen 챗봇 실행**을 한 번 누릅니다. 빌더와 별도인 새 브라우저 창이
   열리지 않으면 화면에 표시된 `127.0.0.1` 주소의 **열려 있는 Qwen 챗봇으로 이동**을 누릅니다.
2. 독립 앱에서 기관과 **대화할 규정**을 고릅니다. `질문 가능`은 활성 청크의 검수 결정이
   모두 끝났고, 승인 청크 수와 색인 레코드 수가 일치하며 stale 기록이 0개라는 뜻입니다.
   `승인·색인 필요` 항목은 빌더의 ③으로 돌아가 완료하기 전까지 질문할 수 없습니다.
3. **Ollama · qwen3:8b 연결 확인**을 누릅니다. `연결되었습니다`가 보이면
   다음으로 갑니다. 실패하면 Ollama 실행 여부와 `qwen3:8b` 설치를 확인하고 다시 누릅니다.

![Qwen3 8B가 준비되고 대화할 규정을 선택한 독립 챗봇 화면](docs/assets/readme-guide-05-qwen-ready.png)

위 화면은 저장소 문서나 실제 기관 식별자가 아닌 **샘플 공공기관·샘플 복무규정** 3개
조항으로 만든 합성 캡처입니다. `승인 조항 3/3`, `검토 대기 0`, `색인 상태 indexed`를
동시에 확인할 수 있습니다.

4. `선택한 규정에 대해 질문하세요` 입력칸에 자연스럽게 질문합니다. 예:
   `이 규정의 목적과 적용 대상을 알려줘.` 첫 답변 뒤에는
   `그 절차에서 담당자는 누구야?`처럼 이어서 물을 수 있습니다. 처리 중에는 질문 분석,
   근거 검색, Qwen3 8B 답변, Qwen3 4B 감사, 인용 검증 단계와 경과 시간이 게이지에 표시됩니다.
...
합성 규정으로 실제 로컬 실행을 점검했을 때 1.7B 질의 분석 → 승인 근거 검색 → 8B 답변 →
4B 주장 감사 → 결정적 인용 검증이 100%까지 완료됐고, 답변에는 제1조·제2조 근거 인용
2건만 연결됐습니다.
