...
PR MCP Builder는 PDF·HWP·HWPX·DOCX로 흩어진 규정을 정리하고, **운영자가 최종
승인한 내용만** ChatGPT·Codex·Claude에서 조회하게 만드는 Windows용
...
Kordoc을 설치한 뒤 새 초안으로 다시 전처리하고, 검수 권고를 확인한 운영자가 다시 승인해야 합니다. 이 재전처리는
...
| ② | `② 결과 확인`의 **요약**에서 **전처리된 글자 확인** → **이슈**와 **표·별표** 확인 → 화면 아래 두 확인란을 순서대로 선택하는 것을 권고한다. 초보자 모드에서는 `정리된 내용(청크)` 탭을 숨긴다. 두 확인란을 선택하지 않아도 ③으로 이동할 수 있다 | ③으로 바로 이동 가능 |
| ③ | (1단계) 규정 디렉터리에서 **규정 열기** → (2단계) **원본·전처리본·AI 검수 의견**을 비교하고 필요하면 가운데 **전처리본 · ✅ 최종본** 칸을 수정(오른쪽 AI 의견은 읽기 전용) → (3단계) **이 규정 최종 확정 · 승인하고 색인** 또는 **선택한 조항 반려**로 처리. 사람 검수를 마치지 않고 최종 확정할 수도 있으며 이 경우 미검수 승인 사유와 감사 이벤트가 자동 기록된다. 여러 규정을 한 번에 끝내려면 **전체 규정 확인**을 켜고 **전체 규정 최종 확정** | 선택한 모든 규정의 **승인·색인 완료** 또는 명시 반려로 처리 방향 결정 |
...
- **이전 단계**와 **다음 단계**는 설명 위치만 바꿉니다. 승인과 색인은 자동으로 실행되지
  않으며 운영자가 최종 버튼을 눌러야 합니다. 원문 확인은 권고되며, 생략하면 감사 기록에 남습니다.
...
![규정 문서의 검수 권고를 확인하고 운영자가 승인한 뒤 로컬 AI와 HTTPS MCP로 연결하는 PR MCP Builder](https://raw.githubusercontent.com/koul777/Public-Regulation-MCP-Builder/main/docs/assets/pr-mcp-builder-hero.png)
...
> 사람 검수를 권고하며, 운영자가 명시적으로 승인한 규정만 MCP 데이터로 생성합니다.
...
| 1. 규정 준비 | 파일 추가 → 결과·검수 권고 확인 → 운영자 승인 | 승인 데이터의 색인 완료 |
...
AI 제안은 참고용입니다. 사람의 원문 확인을 권고하지만, 최종 승인 여부는 운영자가
결정합니다. 최종 버튼으로 명시적으로 승인된 내용만 검색 색인과 MCP에 들어가며,
원문 확인을 생략한 승인은 `approved_without_review` 감사 이벤트로 구분됩니다.
...
- 운영자가 최종 버튼으로 명시적으로 승인한 청크만 공식 색인과 MCP 번들에 들어갑니다.
  사람 검수 완료는 권고 사항이며, 미검수 승인은 사유와 감사 이벤트로 구분됩니다.
...
## 2026년 9월 14일 — 초보자 흐름과 유지보수 경계 개선

- 전처리·결과 확인·승인·연결 단계를 공통 상태로 계산하고, 사이드바에 현재 단계와 다음
  행동을 한국어로 표시합니다. 상태 계산에 문제가 생겨도 안전한 전처리 단계로 돌아갑니다.
- MCP와 로컬 Qwen 연결 진단에서 `등록 필요`, `연결 대기`, `조치 필요`, `오래된 상태`,
  `사용 가능`을 구분하고 상태별 복구 안내를 제공합니다.
- 연결 진단, 준비 상태 변환, 워크플로 계산을 독립 서비스로 분리해 화면 코드와 별도로
  테스트하고 유지보수할 수 있도록 했습니다.
- 빈 경로, 허용되지 않은 경로, 과도하게 큰 설정 파일, 비정상 응답과 비정상 종료를
  성공으로 표시하지 않는 fail-closed 처리를 추가했습니다.
- Local LLM 진단과 Qwen 연결 상태가 같은 준비 상태 계약을 사용하도록 정리했습니다.
- 이번 변경에 대한 집중 테스트 **114개**, 전체 테스트 **3,694개 통과**(`16개 건너뜀`)와
  빌드·릴리스 위생 검사를 완료했습니다.
- 자세한 범위와 보류 항목은 [초보자·유지보수 오케스트레이션 계획](docs/beginner_maintenance_orchestration_plan_ko.md)에서
  확인할 수 있습니다.

## 2026년 8월 31일 — 파일 단위 일괄 최종 확정과 사람 검토 권고

- 한 파일에 승인 대기 조문이 남아 있으면 초보자 모드에서도 **"이 파일의 전체 규정 N개
  최종 확정"** 버튼을 바로 누를 수 있게 했습니다. 이전에는 모든 조문을 하나씩 검수해야만
  버튼이 활성화돼, 큰 합본을 한 번에 승인할 수 없었습니다.
- 검수하지 않은 조문은 기본 사유와 함께 `approved_without_review`(검토 없이 승인)로 감사
  추적에 자동 기록되고, 버튼을 누를 때 **사람 검토 권고 경고**가 항상 표시됩니다(승인을
  막지 않는 안내용).
- 규정을 한 건씩 확정하는 **"이 규정 최종 확정"** 버튼은 기존의 조문별 검수 게이트를 그대로
  유지합니다. 검수를 마친 조문은 `approved_without_review`로 잘못 기록되지 않습니다.
...
`전처리 → 운영자 승인 → 로컬 색인` 결과를 Qwen 챗봇과 MCP가 함께 사용합니다.
...
- 독립 챗봇은 현재 기관에서 전처리한 규정을 목록으로 보여 주되, 운영자 승인·색인이 모두
  끝난 규정만 `질문 가능`으로 표시하고 질문 입력을 허용합니다. 사람 검수는 승인 전에 권고하지만
  완료 여부가 승인 버튼을 막지는 않으며, 미검수 승인은 감사 저널에 별도 기록됩니다.
...
- 파일 등록 → 결과·검수 권고 확인 → 운영자 승인 → Qwen 챗봇·MCP 연결 순서로 안내하되, 승인이나
