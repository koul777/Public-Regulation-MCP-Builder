...
이번에는 처음 사용하는 사람이 화면을 보면서 **지금 해야 할 한 가지를 바로 이해하고,
단계가 바뀌어도 안내를 잃지 않도록** 초보자 모드를 실제 사용 흐름 기준으로 보완했습니다.
...
운영 화면의 ④ 단계는 이제 **Qwen 규정 챗봇**을 첫 탭으로 열며, 질문과 답변을
대화 형태로 유지하고 후속 질문에는 이전 사용자 질문을 검색 문맥으로 반영합니다.

- `Qwen3 8B 챗봇 켜기`를 누르면 로컬 Ollama 설정이 Streamlit 재실행 후에도 유지됩니다.
- Qwen3 8B 답변은 Qwen3 4B 주장 감사와 결정적 인용 검증을 통과해야 그대로 표시됩니다.
...
초보자 모드에서는 첫 화면의 **초보자 안내 시작**을 누른 뒤, 사이드바의
**전체 과정과 담당 모델을 한눈에 보기**를 펼쳐 보세요. 전처리 8단계와 질의·답변
...
![첫 시작 화면에서 로컬 Qwen 챗봇 또는 MCP 연결을 선택하는 안내](docs/assets/readme-guide-05-ai-path-choice.png)

- **이 PC의 로컬 Qwen 챗봇으로 바로 질문**(권장): 승인 뒤 ④에서 Qwen3 8B를 켜고 이
  프로그램 안에서 규정에 질문합니다. 질문과 규정은 외부 API로 보내지 않습니다.
- **ChatGPT·Claude·Codex에 MCP로 연결**: 승인 뒤 ④가 **MCP 생성·외부 AI 연결**로
  표시됩니다. 사용할 앱을 골라 MCP 묶음을 만들고, 그 앱에서 `list_regulations` →
  `search` → `fetch`를 확인합니다. 로컬 Qwen은 옆 탭에서 계속 사용할 수 있습니다.
...
| ④ | **로컬 Qwen 선택:** Qwen3 8B 켜기 → 연결 확인 → 질문 → 답변과 근거 조문 확인. **MCP 선택:** MCP 원리·변환 과정 확인 → 규정 범위 → AI 앱 → 저장 설정·**생성할 MCP 이름 (필수 입력)** → **MCP로 쓸 파일 묶음 만들기** → 앱별 등록·연결 진단 → `list_regulations` → `search` → `fetch` | Qwen은 답변과 근거 조문 확인, MCP는 여섯 개의 실제 연결 확인 완료 |
...
처음에 **로컬 Qwen**을 골랐다면 ④는 `Qwen 규정 챗봇·AI 연결`로 보이고 Qwen 탭부터
따라가면 됩니다. **MCP 연결**을 골랐다면 ④는 `MCP 생성·외부 AI 연결`로 보이고 MCP
탭부터 시작합니다. 어느 경우에도 다른 탭과 기존 MCP 기능은 남아 있습니다. MCP를 쓸
...
### 1-6. 로컬 Qwen으로 처음 질문하기 — 다섯 단계

첫 화면에서 **이 PC의 로컬 Qwen 챗봇으로 바로 질문**을 골랐다면, 승인·색인이 끝난 뒤
④에서 아래 다섯 단계만 차례대로 진행하세요. 이 과정은 MCP를 만들지 않으며, MCP 기능을
지우지도 않습니다. Qwen과 MCP는 같은 승인 로컬 RAG를 공유하므로 별도 RAG 구축은
필요 없습니다.

1. `④ Qwen 규정 챗봇·AI 연결`을 엽니다. **승인·색인된 규정 준비 상태**가 보이지 않으면
   아직 ③이 끝나지 않은 것이므로 승인·색인부터 마칩니다.
2. **Qwen3 8B 챗봇 켜기**를 누릅니다. 처음에는 모델 없는 근거 답변으로 보일 수 있으며,
   이 버튼은 이 PC의 Ollama와 `qwen3:8b`를 사용하도록 바꿉니다.
3. **Qwen 연결 확인**을 누릅니다. `이 PC의 Qwen이 응답할 준비가 됐습니다`가 보이면
   다음으로 갑니다. 실패하면 Ollama 실행 여부와 `qwen3:8b` 설치를 확인하고 다시 누릅니다.

![Qwen3 8B가 준비되고 연결 확인을 마친 ④ 화면](docs/assets/readme-guide-05-qwen-ready.png)

4. **예시 질문** 하나를 누르거나 `승인된 규정에 관해 질문하세요` 입력칸에 자연스럽게
   질문합니다. 예: `이 규정의 목적과 적용 대상을 알려줘.` 첫 답변 뒤에는
   `그 절차에서 담당자는 누구야?`처럼 이어서 물을 수 있습니다.
5. 답변만 읽고 끝내지 말고, 아래에 열린 **근거 조문**에서 규정명·조문 번호·인용문을
   원문과 함께 확인합니다. 실제 답변은 Qwen3 8B가 만들지만, Qwen3 4B의 주장 감사와
   결정적 인용 검증을 통과한 근거만 표시됩니다. 검증이 안 되면 그럴듯한 답을 내보내는
   대신 승인된 근거의 발췌와 인용으로 제한됩니다.

![Qwen 답변과 함께 근거 조문 및 인용을 확인하는 화면](docs/assets/readme-guide-05-qwen-answer.png)
